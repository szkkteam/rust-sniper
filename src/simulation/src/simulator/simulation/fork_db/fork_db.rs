    ...
    fn basic(&mut self, address: B160) -> Result<Option<AccountInfo>, Self::Error> {
        ...
        match self.db.accounts.get(&address) {
            ...
            None => {
                ...
                // keeep record of fetched acc basic info
                if info.is_some() {
    ...
    fn storage(&mut self, address: B160, index: rU256) -> Result<rU256, Self::Error> {
        ...
        if let Some(account) = self.db.accounts.get(&address) {
            if let Some(entry) = account.storage.get(&index) {
                // Account storage exists in slot
                return Ok(*entry)
            }
        }
        ...
        // Keep record of fetched storage (can unwrap safely as CacheDB always return true)
...
impl DatabaseRef for ForkDB {
    ...
    fn code_by_hash(&self, code_hash: B256) -> Result<rBytecode, Self::Error> {
        match self.db.code_by_hash(code_hash) {
            Ok(code) => Ok(code),
            Err(_) => Err(DatabaseError::MissingCode(code_hash))
        }
    }
}
