...
// Decode the result of the braindance contract's calculateSwapV2 function
pub fn decode_maxbuy_swap_v2_result(output: Bytes) -> Result<U256, AbiError> {
    let braindance_contract = BaseContract::from(parse_abi(&[
        "function maxbuySwapV2(uint amountOut, address targetPair, address inputToken, address outputToken) external returns (uint realAfterBalance)",
    ]).unwrap());

    braindance_contract.decode_output("maxbuySwapV2", output)
}
